...
class SIS_truncate(object):
    ...
    def function(self, x, y, theta_E_trunc, r_trunc, center_x=0, center_y=0):
        ...
        if isinstance(r, int) or isinstance(r, float):
            if r < r_trunc:
                ...
                f_ = theta_E_trunc * r_trunc + 1. / 2 * theta_E_trunc * (3 - r / r_trunc) * (r - r_trunc)
            else:
                f_ = 3./2 * theta_E_trunc * r_trunc
        else:
            f_ = np.zeros_like(r)
            f_[r < r_trunc] = theta_E_trunc * r[r < r_trunc]
            r_ = r[(r < 2*r_trunc) & (r > r_trunc)]
            f_[(r < 2*r_trunc) & (r > r_trunc)] = theta_E_trunc * r_trunc + 1. / 2 * theta_E_trunc * (3 - r_ / r_trunc) * (r_ - r_trunc)
    ...
    def _dphi_dr(self, x, y, theta_E_trunc, r_trunc):
        """
        ...
        if isinstance(r, int) or isinstance(r, float):
            if r == 0:
                ...
                a = theta_E_trunc * (2 - r / r_trunc)
            else:
                a = 0
        else:
            a = np.zeros_like(r)
            a[(r < r_trunc) & (r > 0)] = theta_E_trunc
            r_ = r[(r < 2*r_trunc) & (r >= r_trunc)]
            a[(r < 2*r_trunc) & (r >= r_trunc)] = theta_E_trunc * (2 - r_ / r_trunc)
            a[r >= 2*r_trunc] = 0
        return a

    def _d2phi_dr2(self, x, y, theta_E_trunc, r_trunc):
        """
        ...
        if isinstance(r, int) or isinstance(r, float):
            if r < r_trunc:
                ...
                a = -theta_E_trunc / r_trunc
            else:
                a = 0
        else:
            a = np.zeros_like(r)
            a[r < r_trunc] = 0
            a[(r < 2*r_trunc) & (r > r_trunc)] = -theta_E_trunc / r_trunc
            a[r > 2*r_trunc] = 0
        return a
